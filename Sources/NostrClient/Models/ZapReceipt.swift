...
public struct ZapReceipt: Sendable, Hashable {
    ...
    public var zapRequest: Event? {
        guard let descriptionJSON else { return nil }
        return try? JSONDecoder().decode(Event.self, from: Data(descriptionJSON.utf8))
    }
...
extension ZapReceipt {
    ...
    public func validate(lnurlProviderPubkey: String, expectedAmountMillisats: Int64? = nil) throws {
        ...
        // 3. The invoice must parse.
        guard let invoice = Bolt11Invoice(bolt11) else { throw ValidationError.invalidBolt11 }

        // 4. The signature confirms the provider really issued this receipt — the trust anchor for
        //    everything below.
        ...
        // 7. A preimage, when present, must hash to the invoice's payment hash.
